...
public class FineAdjustment implements Comparable<FineAdjustment> {
    ...
    public static FineAdjustment create(Mat mat, int left, int up, int rWidth, int rHeight) {
        FineAdjustment theAdjustment = new FineAdjustment(0,0, Score.create(mat,left, up, rWidth, rHeight));
        for (int x = left- XDELTA; x < left+XDELTA ; x++) {
            for (int y = up- YDELTA; y < up+YDELTA ; y++) {
                Score currentScore = Score.create(mat,x, y, rWidth, rHeight);
    ...
    private static class Score implements Comparable<Score>{
        ...
        public static Score create(Mat mat, int left, int up, int rWidth, int rHeight) {
            ...
            Core.inRange(subMat, ImageProcessor.param_LOW_BOUND_BLACKS_COUNTING, ImageProcessor.param_UPPER_BOUND_BLACKS_COUNTING, subMat);
